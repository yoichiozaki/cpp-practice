...
int main()
{
    Point3D uza, enemy;
    Plane3D barrier;
    cin >> uza.x >> uza.y >> uza.z;
    cin >> enemy.x >> enemy.y >> enemy.z;
    cin >> barrier.p1.x >> barrier.p1.y >> barrier.p1.z;
    cin >> barrier.p2.x >> barrier.p2.y >> barrier.p2.z;
    cin >> barrier.p3.x >> barrier.p3.y >> barrier.p3.z;
    Line3D l(uza, enemy);
    Polygon3D cps = crossPoint3DSPL(l, barrier);
    if (cps.empty())
        cout << "HIT" << endl;
    else
    {
        Point3D cp = cps[0];
        if (EPS < area(cp, barrier.p2, barrier.p3) + area(cp, barrier.p3, barrier.p1) + area(cp, barrier.p1, barrier.p2) - area(barrier.p1, barrier.p2, barrier.p3))
            cout << "HIT" << endl;
        else
            cout << "MISS" << endl;
    }
